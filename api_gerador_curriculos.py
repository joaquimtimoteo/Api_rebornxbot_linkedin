...
app = FastAPI(title="Sistema de Usuários com Ativação", version="1.0.0")
router = APIRouter()
...
def gerar_curriculo_pdf(nome, local, telefone, email, experiencia, habilidades, educacao, projetos, caminho_arquivo):
...
async def gerar_curriculo(curriculo: UserBase, background_tasks: BackgroundTasks):
...
async def obter_curriculo(nome: str):
